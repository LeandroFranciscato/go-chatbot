...
func (router router) chatReview(group *gin.RouterGroup) {
	group.POST("review/customer/:customerID/order/:orderID", func(c *gin.Context) {
		router.reviewChatHandler(c, router.ReviewFlow)
	})
}

func (router router) reviewChatHandler(c *gin.Context, flow flow.Flow) {
	// retrieve chat history
	customerID := c.Param("customerID")
	orderID := c.Param("orderID")
	chatHistory, err := flow.GetHistory(c, customerID, orderID)
	if err != nil {
	...
	if chatHistory.CurrentStep > step {
		c.HTML(http.StatusOK, "chat_review.html", gin.H{
			"title":       flow.Name(),
			"step":        strconv.Itoa(chatHistory.CurrentStep),
			"customerID":  customerID,
			"orderID":     orderID,
			"final":       chatHistory.CurrentStep == flow.FinalStep(),
			"order":       orderStr,
	...
	err = flow.SaveHistory(c, nextStep, customerID, orderID, chatHistoryStr)
	if err != nil {
